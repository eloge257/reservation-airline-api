...
class Validation {
  ...
  async required(key, intitialValue) {
    try {
      ...
      if (isInvalid) {
        this.setError(
          key,
          this.customMessages?.[key]?.required || `Ce champ est obligatoire`
        );
      }
    } catch (error) {
      throw error;
    ...
    if (min && !max && value.length < min) {
      this.setError(
        ...
        this.customMessages?.[key]?.length ||
          `Entrez au moins ${min} caractères`
      );
  ...
  async fileTypes(key, value, params) {
    ...
    if (!VALID_MIMES.includes(value.mimetype)) {
      this.setError(
        key,
        this.customMessages?.[key]?.fileTypes ||
          `Invalid file type(${params.join(", ")})`
      );
    }
  }
  ...
  async fileSize(key, value, params) {
    ...
    if (params < value.size) {
      ...
      this.setError(
        key,
        this.customMessages?.[key]?.fileSize ||
          `File too large (max: ${megaBite} MB)`
      );
    }
  }
  ...
  alpha(key, value) {
    ...
    if (!isString) {
      this.setError(
        key,
        this.customMessages?.[key]?.alpha ||
          `This field must contains alphanumeric characters only`
      );
    }
  }
  number(key, value) {
    ...
    if (!isnum) {
      this.setError(
        key,
        this.customMessages?.[key]?.number ||
  ...
  date(key, value, params) {
    ...
    if (!isDate) {
      this.setError(
        key,
        this.customMessages?.[key]?.date ||
          `This field must be a valid date(${format})`
      );
    }
  }
  /**lm!
  ...
  /**
   * Retrouver tous les données validées. A être appelé après run()
   * @returns Object
   */
  getValidated() {
    const data = {};
